...
def resize_image(image, width, height):#图像padding
...
def drawbbox(image, bbox, color=None, thickness=2, textcolor=(0, 0, 0), landmarkcolor=(0, 0, 255)):
    ...
    for i in range(len(bbox.landmark)):
        landmark_list.append([data for data in bbox.landmark[i][:2]])
    for i in range(len(bbox.landmark)):
        landmark_list[i][0] = int(landmark_list[i][0]-x)
        landmark_list[i][1] = int(landmark_list[i][1]-y)

    landmark_list = np.array(landmark_list)     #人脸五个点
    cropped = image[int(y):int(y+h), int(x):int(x+w)]   # 裁剪坐标为[y0:y1, x0:x1]
    cropped, _ = Alignment_1(cropped, landmark_list)    #人脸校正
    cropped = resize_image(cropped,160,160)
    # cv2.imwrite("./save_img/{}.jpg".format(1),cropped)    #保存图片
    img = Image.fromarray(cv2.cvtColor(cropped, cv2.COLOR_BGR2RGB)) #转换为PIL数据，方便输入网络
    img = tran(img)     #torch的数据处理
    img.unsqueeze_(dim=0)   #增加一个维度

    mobilenet = torch.load('./logs/params.pth')
    mobilenet.eval()
    out = mobilenet(img)

    out = torch.argmax(out,dim=1)
    out = int(out)
    mask_list = ['mask','nomask']   #是否口罩具的列表
    mask_data = mask_list[out]      #返回是否带口罩
    print(mask_data)

    #text = f"{bbox.label} {bbox.score:.2f}"
    text = f"{bbox.score:.2f}"+' {}'.format(mask_data)
